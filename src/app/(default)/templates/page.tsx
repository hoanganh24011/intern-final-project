...
const TemplatesPage = () => {
			...
			<div className="flex justify-center">
				...
				{openRowMenu === template.id && (
					<div className="absolute right-0 z-10 mt-10 w-40 rounded-xl border border-gray-200 bg-white px-2 py-2 shadow-md">
						...
						>
							<SquarePen size={15} />
							<span className="font-medium">{t("templates_page.edit")}</span>
						</button>
						<button
							type="button"
						...
						>
							<Trash2 size={15} />
							<span className="font-medium">{t("templates_page.delete")}</span>
						</button>
					</div>
				)}
			</div>
			...
			rows.push(
				...
				>
					<td className="px-2 py-2 text-center">
						{/* Area: Checkbox for selecting a template */}
						<input
							type="checkbox"
	...
	function TopRightNotification({
		...
		// Area: Effect for auto-close notification
		useEffect(() => {
			if (!open) return;
				...
				>
					<div className="flex min-h-screen items-center justify-center bg-black/20">
						<Dialog.Panel className="h-fit w-full max-w-screen-md rounded-xl bg-white p-6 shadow-lg">
							...
							<div className="mb-6">
								<TextArea
									label={"templates_page.description"}
									value={addContent}
